<template>
    <transition name="basket">
        <div class="basket__product basket__click" id="basket__click">
            <div class="py-2">
                <!-- BASKETITEMS -->
                <b-overlay :show="$store.getters['loadItemShow'].itemBasket" rounded="sm">
                    <Item v-for="item of this.$store.getters['basket/basket']" type="basket" :item="item"
                          :key="item.id"/>
                </b-overlay>
            </div>
            <div class="row mx-3 d-flex justify-content-between my-2 ">
                ...
                <router-link tag="button" :to="{name: 'ShoppingCart'}"
